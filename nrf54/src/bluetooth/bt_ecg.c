/**
 * @file bt_ecg.c
 * @brief ECG Bluetooth channel: queue, batching, DLQ, and stats.
 */
...
static ecg_data_t ecg_data_value = { 0 };
static ecg_data_t ecg_batch_buffer[BT_BATCH_SIZE];
...
bt_channel_t ecg_channel = {
    ...
    .batch_elem_size = sizeof(ecg_data_t),
    .dlq = &ecg_dlq,
    .dlq_size = DLQ_SIZE,
...
void data_ccc_changed(const struct bt_gatt_attr *attr, uint16_t value) {
    ...
    LOG_INF("ECG data notifications %s", data_notify_enabled ? "enabled" : "disabled");
}

ssize_t read_ecg_data(struct bt_conn *conn, const struct bt_gatt_attr *attr,
    void *buf, uint16_t len, uint16_t offset) {
    return bt_gatt_attr_read(conn, attr, buf, len, offset,
        &ecg_data_value, sizeof(ecg_data_value));
}
...
void bt_ecg_process_sample(const bt_sample_data_t *sample) {
    ...
    ecg_data_t *slot = ecg_batch_buffer + ecg_batch_count;
    slot->sample_count = sample->timestamp;
    slot->ecg_aux = sample->ecg_aux;
    ecg_batch_count++;
...
int bluetooth_queue_sample(int32_t heart, int32_t ecg_aux, uint32_t timestamp) {
    ...
    if (err == 0) {
        (void) bluetooth_queue_raw_ecg_sample(heart, timestamp);
    }
#endif
    return err;
}
